...
function UserSettings() {
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            try {
                const response = await fetch('https://api.clipr.solutions:5000/user/settings', {
                    method: 'GET',
                    headers: {
    ...
    const handleSoftwareSettingsSubmit = async (e) => {
        ...
        try {
            const response = await fetch('https://api.clipr.solutions:5000/update/clipr/software/', {
                method: 'POST',
                headers: {
